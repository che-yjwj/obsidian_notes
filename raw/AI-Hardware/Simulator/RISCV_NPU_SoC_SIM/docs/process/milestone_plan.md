...
- architecture semantics, IR/CMDQ contract, timing, quantization의 정의는 spec/design 문서군을 우선한다.
- process control-plane은 workflow/roadmap 문서군을 우선한다.
- 정확한 문서 경로와 최신 canonical entry는 `docs/README_SPEC.md`와 `docs/process/project_roadmap.md`에서 찾는다.
...
- 상위 개요와 spec index가 존재한다.
- spec / design / process / test 문서군이 역할별로 분리되어 있다.
- 구현은 이 문서군을 기준으로 점진적으로 코드화하는 단계에 있다.
...
- [ ] (P0-IR-CORE-001) `compiler/ir/core_types.py`에 IR / Tensor / Quant / CMDQ / Timing / Trace dataclass 골격 정의
  - 필드 이름 및 기본 구조는 IR / ISA / timing / trace spec군과 일치시킨다.
...
- [ ] (P1-C-CMDQ-001) CmdqGeneratorPass: TE MatMul CMDQ 생성
  ...
  - 세부 포맷과 의미론은 ISA/CMDQ spec와 generator design 문서군을 따른다.
...
- [ ] SPM timing/bank 모델 구현 (`simulator/memory/spm_model.py`)
  - timing/memory spec군의 기준식을 우선한다.
...
- [ ] Conv/Attention용 CMDQ opcode 및 args 정의 (`opcode_set_definition.md` 연계)
  - opcode naming과 인자 구조는 ISA spec군에서 확정한다.
...
(세부 Task는 기존 초안과 동일한 구조이되, 실제 구현 시 quantization/workload spec군과 연계)
...
이 Milestone Plan은 구현/PR/테스트 계획을 추적하는 운영용 문서다.  
정확한 구조 정의는 roadmap와 spec/design 문서군에서 유지한다.
